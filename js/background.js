...
chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
  ...
  // Handle network info request from dashboard
...
// Get network info for dashboard
...
// Store provider information for dashboard
    ...
    if (data.org) {
      ...
      chrome.storage.local.get('providerHistory', (storage) => {
        ...
        chrome.storage.local.set({ providerHistory: recentProviders }, () => {
          console.log('Provider history updated:', provider);
        });
      });
    }
  } catch (error) {
...
// Store dashboard data with provider info
async function storeDashboardData(results, timestamp) {
  return new Promise((resolve) => {
    chrome.storage.local.get(['dashboardData', 'providerHistory'], (storage) => {
      ...
      const providerHistory = storage.providerHistory || [];
      const providerMap = new Map();
      
      providerHistory.forEach(entry => {
        if (entry.provider) {
      ...
      dashboardData.providerHistory = Array.from(providerMap.entries());
      
      // Keep only recent entries (max 50)
      const maxEntries = 50;
      Object.keys(dashboardData).forEach(key => {
        if (Array.isArray(dashboardData[key]) && dashboardData[key].length > maxEntries) {
...
function handleOnlineState() {
  ...
  chrome.storage.local.get(['onlineStartTime'], (data) => {
    if (!data.onlineStartTime) {
      chrome.storage.local.set({ onlineStartTime: Date.now() });
    }
  });
